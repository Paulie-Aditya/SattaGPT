...
const AGENTS = {
  vedant: { name: "Vedant", party: "Hindu Nationalist", avatar: "🕉️", color: "bg-orange-500" },
  samira: { name: "Samira", party: "Secular Progressive", avatar: "🎓", color: "bg-blue-500" },
  ravi: { name: "Ravi", party: "Marxist Revolutionary", avatar: "✊", color: "bg-red-500" },
  fatima: { name: "Fatima", party: "Grassroots Socialist", avatar: "🤝", color: "bg-green-500" },
  gurmeet: { name: "Gurmeet", party: "Sikh Federalist", avatar: "🪯", color: "bg-yellow-500" },
  neha: { name: "Neha", party: "Techno-liberal", avatar: "💻", color: "bg-purple-500" },
}
...
interface AnalyticsData {
  ...
  roundPerformance: Array<{ round: number; agent1Gain: number; agent2Gain: number }>
}
...
export default function SattaGPT() {
  ...
  const [currentSpeaker, setCurrentSpeaker] = useState<keyof typeof AGENTS>("vedant")
  ...
  const [liveVotes, setLiveVotes] = useState({ agent1: 0, agent2: 0, total: 0 })
  const [userVote, setUserVote] = useState<"agent1" | "agent2" | null>(null)
  const [voteSurge, setVoteSurge] = useState<"agent1" | "agent2" | null>(null)
  const [previousPercentages, setPreviousPercentages] = useState({ agent1: 0, agent2: 0 })
  ...
  const { messages, append, isLoading, setMessages } = useChat({
    ...
    onFinish: () => {
      ...
      setLiveVotes((prev) => {
        const newVotes = {
          agent1: prev.agent1 + (speakingAgent === "agent1" ? surgeVotes : backgroundVotes),
          agent2: prev.agent2 + (speakingAgent === "agent2" ? surgeVotes : backgroundVotes),
        ...
        setAnalytics((prevAnalytics) => ({
          ...
          roundPerformance: [
            ...
            {
              round: roundCount + 1,
              agent1Gain: speakingAgent === "agent1" ? surgeVotes : backgroundVotes,
              agent2Gain: speakingAgent === "agent2" ? surgeVotes : backgroundVotes,
            },
          ],
        }))

        // Track strongest argument
        const voteGain = speakingAgent === "agent1" ? surgeVotes : backgroundVotes
        setAnalytics((prevAnalytics) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setLiveVotes((prev) => {
        ...
        setVotingRateHistory((prevRates) => [...prevRates.slice(-19), currentRate])
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const currentVotingRate = votingRateHistory.length > 0 ? votingRateHistory[votingRateHistory.length - 1] : 0
      ...
      setAnalytics((prev) => ({
        ...
        engagementScore: Math.min(100, liveVotes.total / 10 + messages.length * 5 + roundCount * 10),
        momentumShifts:
          prev.votingPattern.length > 1
            ? prev.votingPattern.reduce((shifts, current, index) => {
                if (index === 0) return 0
                const prevPoint = prev.votingPattern[index - 1]
                const leadChanged = current.agent1 > current.agent2 !== prevPoint.agent1 > prevPoint.agent2
    ...
    return () => clearInterval(interval)
  }, [debateStarted, liveVotes, messages, roundCount, votingRateHistory, responseTimes])

  const startDebate = async () => {
    if (!selectedTopic) return

    const actualTopic = selectedTopic.startsWith("custom:") ? selectedTopic.slice(7) : selectedTopic
  ...
  useEffect(() => {
    if (debateStarted && !isLoading && messages.length > 0 && roundCount < 6 && roundCount > 0) {
  ...
  const agent1Percentage = liveVotes.total > 0 ? (liveVotes.agent1 / liveVotes.total) * 100 : 50
  const agent2Percentage = liveVotes.total > 0 ? (liveVotes.agent2 / liveVotes.total) * 100 : 50
  ...
  const leadingAgent = agent1Percentage > agent2Percentage ? "agent1" : "agent2"
        ...
        {!debateStarted ? (
          ...
          <div className="max-w-3xl mx-auto">
            <Card className="bg-slate-800/50 border-red-800/50">
              <CardHeader>
                <CardTitle className="text-white text-xl">Setup Debate</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {/* Topics */}
                <div>
                  <h3 className="text-red-200 mb-3 text-sm font-medium">What topic do you want to see debated?</h3>
                  ...
                  <div className="mb-3">
                    <input
                      ...
                      value={selectedTopic.startsWith("custom:") ? selectedTopic.slice(7) : ""}
                      onChange={(e) => setSelectedTopic(e.target.value ? `custom:${e.target.value}` : "")}
                      className="w-full p-2 rounded text-sm bg-slate-700/50 text-slate-100 placeholder-slate-400 border border-slate-600/50 focus:border-red-500 focus:outline-none"
                    />
                  </div>

                  {/* Suggested Topics */}
                  <div className="space-y-1">
                    <p className="text-slate-400 text-xs mb-2">Or choose from trending topics:</p>
                    {TOPICS.map((topic, i) => (
                      <button
                        key={i}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <h3 className="text-red-200 mb-2 text-sm font-medium">Left:</h3>
                    <div className="space-y-1">
                  ...
                  <div>
                    <h3 className="text-red-200 mb-2 text-sm font-medium">Right:</h3>
                    <div className="space-y-1">
                      {Object.entries(AGENTS).map(([key, agent]) => (
                        <button
                          key={key}
                ...
                <Button
                  onClick={startDebate}
                  disabled={!selectedTopic || (selectedTopic.startsWith("custom:") && !selectedTopic.slice(7).trim())}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            ...
            <div className="lg:col-span-3">
              <Card className="bg-slate-800/50 border-red-800/50">
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="text-white text-lg leading-tight">
                        {selectedTopic.startsWith("custom:") ? selectedTopic.slice(7) : selectedTopic}
                      </CardTitle>
                      <div className="flex items-center gap-3 mt-2 text-sm">
                        <span className="flex items-center gap-1">
                          <span>{AGENTS[agent1].avatar}</span>
                          <span className="text-slate-300">{AGENTS[agent1].name}</span>
                        </span>
                        <span className="text-red-400">vs</span>
                        <span className="flex items-center gap-1">
                          <span>{AGENTS[agent2].avatar}</span>
                          <span className="text-slate-300">{AGENTS[agent2].name}</span>
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge className="bg-red-700 text-white text-xs">
                        {Math.ceil(roundCount / 2) + (roundCount === 0 ? 1 : 0)}/3
                      </Badge>
                      <Button
                      ...
                      <Button onClick={resetDebate} variant="ghost" size="sm" className="text-red-400 hover:text-white">
                ...
                <CardContent>
                  <div className="space-y-3 max-h-80 overflow-y-auto" ref={messagesContainerRef}>
                    {messages
                      ...
                      .map((message, index) => {
                        ...
                        return (
                          <motion.div
                            key={message.id}
                            className={`flex ${isAgent1Turn ? "justify-start" : "justify-end"}`}
                          ...
                          >
                            <div className="max-w-[80%]">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-lg">{AGENTS[currentAgent].avatar}</span>
                                <span className="text-sm font-medium text-white">{AGENTS[currentAgent].name}</span>
                                <AnimatePresence>
                                  {isSurging && (
                                    <motion.div
                              ...
                              <motion.div
                                ...
                                animate={isSurging ? { scale: [1, 1.02, 1] } : {}}
                    ...
                    {isLoading && (
                      <div className="flex justify-center">
                        <div className="flex items-center gap-2 text-red-400">
                          ...
                          <span className="text-sm">{AGENTS[currentSpeaker].name} typing...</span>
                      ...
                      <CardContent>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                          <div className="bg-slate-700/30 p-3 rounded-lg">
                            ...
                            <div className="text-lg font-bold text-white">{analytics.engagementScore.toFixed(0)}%</div>
                          </div>

                          <div className="bg-slate-700/30 p-3 rounded-lg">
                            <div className="flex items-center gap-2 mb-1">
                              <Users className="w-4 h-4 text-blue-400" />
                              <span className="text-xs text-slate-400">Voting Rate</span>
                            </div>
                            <div className="text-lg font-bold text-white">{currentVotingRate.toFixed(1)}/s</div>
                          </div>
                          ...
                          <div className="bg-slate-700/30 p-3 rounded-lg">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-xs text-slate-400">Momentum Shifts</span>
                            </div>
                            <div className="text-lg font-bold text-white">{analytics.momentumShifts}</div>
                          </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="bg-slate-700/30 p-3 rounded-lg">
                            <h4 className="text-sm font-medium text-white mb-2">Round Performance</h4>
                            <div className="space-y-2">
                              {analytics.roundPerformance.map((round, index) => (
                                <div key={index} className="flex items-center justify-between text-xs">
                                  <span className="text-slate-400">Round {round.round}</span>
                                  <div className="flex gap-2">
                                    <span className="text-orange-400">
                                      {AGENTS[agent1].avatar} +{round.agent1Gain}
                                    </span>
                                    <span className="text-blue-400">
                                      {AGENTS[agent2].avatar} +{round.agent2Gain}
                                    </span>
                                  </div>
                                </div>
                              ))}
                            ...
                            <div className="space-y-2 text-xs">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-slate-300">
                                  Peak voting: {analytics.peakVotingRate.toFixed(1)} votes/sec
                                </span>
                              </div>
                              {analytics.strongestArgument.agent && (
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-slate-300">
                                    Strongest: {analytics.strongestArgument.agent} R{analytics.strongestArgument.round}
                ...
                <CardHeader className="pb-3">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-1 text-slate-400">
                      ...
                      <span className="font-mono">{liveVotes.total.toLocaleString()}</span>
                    </div>
                    {leadMargin > 1 && (
                      <motion.div
                        ...
                        animate={{ opacity: 1 }}
                      >
                        <span className="text-xs">Lead: {leadMargin.toFixed(1)}%</span>
                      </motion.div>
                    )}
                  </div>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <motion.button
                      ...
                      animate={voteSurge === "agent1" ? { scale: [1, 1.05, 1] } : {}}
                      transition={{ duration: 0.5 }}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span>{AGENTS[agent1].avatar}</span>
                          <span className="font-medium">{AGENTS[agent1].name}</span>
                    ...
                    <motion.button
                      ...
                      animate={voteSurge === "agent2" ? { scale: [1, 1.05, 1] } : {}}
                      transition={{ duration: 0.5 }}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span>{AGENTS[agent2].avatar}</span>
                          <span className="font-medium">{AGENTS[agent2].name}</span>
                  ...
                  <div className="text-xs text-slate-500 pt-2 border-t border-slate-700 space-y-1">
                    <div className="flex justify-between">
                      <span>Status:</span>
                      <span className={isDebateComplete ? "text-green-400" : "text-yellow-400"}>
                        {isDebateComplete ? "Complete" : "Live"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Rate:</span>
                      <span className="text-blue-400">{currentVotingRate.toFixed(1)}/s</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Engagement:</span>
                      <span className="text-green-400">{analytics.engagementScore.toFixed(0)}%</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
